...
export default function PlayersScreen({
  ...
  async function pickPhoto(event) {
    ...
    try {
      ...
      setError('Це фото не вдалося завантажити — спробуй інше.')
    }
  }
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="mx-auto flex max-w-xl flex-col gap-6 p-6">
        <div>
          <h2 className="font-display text-2xl font-bold">Гравці</h2>
          <p className="mt-1 text-sm text-white/55">Від 2 до 8 гравців — співатимете по черзі.</p>
        </div>
        ...
        <form onSubmit={submit} className="card flex flex-col gap-4 p-5">
          ...
          <div>
            <p className="section-label mb-2">Аватар</p>
            ...
            <p className="mt-2 text-xs text-white/35">Фото зберігається лише на цьому пристрої.</p>
          </div>

          <button type="submit" className="btn-primary">Додати гравця</button>
          {error && <p className="text-sm text-red-400">{error}</p>}
        </form>

        {players.length === 0 ? (
          <p className="text-center text-sm text-white/35">Поки що нікого немає. Хто перший?</p>
        ) : (
          <div>
            <p className="section-label mb-2">Склад вечірки · {players.length}</p>
                  ...
                  {openSongsId === player.id && (
                    <div className="flex flex-col gap-3 border-t border-line bg-black/20 p-4">
                      <p className="text-xs text-white/45">
                        Особисті пісні гравця {player.name} — потім одним натиском підуть у чергу.
                      </p>
                        ...
                        <ul className="flex flex-col gap-2">
                          {player.songs.map((song, index) => (
                            <li key={song.id} className="flex items-center gap-2 rounded-xl bg-black/30 p-2">
                              ...
                              >
                                <ListPlus size={14} strokeWidth={2} /> До черги
                              </button>
                              <button
        ...
        {players.length === 1 && (
          <p className="text-center text-sm text-white/35">Додай ще хоча б одного — самому співати сумно.</p>
        )}
      </div>
    </div>
  )
